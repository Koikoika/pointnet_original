...
indoor3d_data_dir = os.path.join(data_dir, 'dso_indoor3d')
NUM_POINT = 4096
H5_BATCH_SIZE = 1000
...
# Set paths
filelist = os.path.join(BASE_DIR, 'meta/all_data_label_original.txt')
data_label_files = [os.path.join(indoor3d_data_dir, line.rstrip()) for line in open(filelist)]
output_dir = os.path.join(data_dir, 'indoor3d_sem_seg_hdf5_data_original')
...
for i, data_label_filename in enumerate(data_label_files):
    ...
    data, label = indoor3d_util_original.room2blocks_wrapper_normalized(data_label_filename, NUM_POINT, block_size=1.0, stride=0.5,
